...
<body>
	...
	<br>
		...
		<table>
			...
			<tbody>		
				<c:forEach items="${listOne}" var="listOne">
					<tr>    
						<td class="checkbox">
							<input name="RowCheck" class="selectRow" type="checkbox" value="${listOne.addr_no}"/></td>
						...
						<td><a class="move" href='<c:out value="${listOne.m_no}"/>'
							data-addr_nickname='<c:out value="${listOne.addr_nickname}"/>'
							...
							data-recipient_name='<c:out value="${listOne.recipient_name}"/>'
							data-recipient_phone='<c:out value="${listOne.recipient_phone}"/>'>
								<c:out value="${listOne.postnum}"></c:out> 
								<c:out value="${listOne.address}"></c:out> 
								<c:out value="${listOne.detailed_address}"></c:out>
							</a>
						</td>
		...
		<div id="btnCss">
			...
			<input type="button" value="주소지 불러오기" class="button button4" onclick="addrMove()">
